...
from loss import CategoryLoss, BinaryLoss
import os
...
class Zi2ZiModel:
    def __init__(self, input_nc=3, embedding_num=40, embedding_dim=128,ngf=64, ndf=64,
                 Lconst_penalty=15, Lcategory_penalty=1, L1_penalty=100,
                 schedule=10, lr=0.001, gpu_ids=None, save_dir='.', is_training=True, image_size=256):
        if is_training:
    ...
    def update_lr(self):
        ...
        for p in self.optimizerD.param_groups:
            current_lr = p['lr']
        ...
        for p in self.optimizerG.param_groups:
            current_lr = p['lr']
            update_lr = current_lr / 2.0
            # minimum learning rate guarantee
            update_lr = max(update_lr, 0.0002)
